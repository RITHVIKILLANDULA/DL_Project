...
\paragraph{Trained checkpoints.} All small model checkpoints (TF--IDF + LogReg, BiRNN seeds, BiLSTM seeds) are committed under \texttt{models/} in the GitHub repository. The DistilBERT weights file (\texttt{model.safetensors}, $\approx$256\,MB) exceeds GitHub's 100\,MB per-file limit and is hosted as a release asset at \url{https://github.com/RITHVIKILLANDULA/DL_Project/releases/tag/v1.0-models}; running \texttt{python scripts/download\_trained\_models.py} fetches it into the correct path. GloVe 6B 100d ($\approx$330\,MB) is auto-downloaded from Stanford's official URL by \texttt{src/fake\_news/embeddings.py} on first use.
...
Two demo interfaces are included in the repository: a Streamlit app (\texttt{app/streamlit\_app.py}) and a Flask app (\texttt{app/app\_flask.py}). Both load all three trained checkpoints (BiRNN, BiLSTM, DistilBERT), expose a model-switcher dropdown, and let the user paste arbitrary text to see the predicted class and probabilities. Screenshots of the running Flask UI under each model are included in \texttt{report/demo\_screenshots/} in the repository (Figures~\ref{fig:demo-distilbert-clickbait}--\ref{fig:demo-rnn}).
...
\label{fig:demo-rnn}
\end{minipage}
\caption*{Model switching on the same input ``police confirm that the missing child has been found safe.'' All three models misclassify this short statement as fake (DistilBERT 83.9\%, BiLSTM 60.9\%, BiRNN 79.4\%) -- a failure mode discussed in \S5: short declarative statements with strong-attribution verbs are over-predicted as fake because the training mix is LIAR-dominated.}
\end{figure}
